...
from depth_prior.lib.models.Surface_normal import surface_normal_from_depth, vis_normal
"""
...
class EdgeguidedNormalRegressionLoss(nn.Module):
    ...
    def forward(self, pred_depths, gt_depths, images, focal_length):
        ...
        edges_mask_dilate = torch.clamp(torch.nn.functional.conv2d(edges_depth_mask.float().to(pred_depths.device), self.kernel.to(pred_depths.device), padding=(1, 1)), 0,
                                       1).bool()
        ...
        # initialization
        loss = torch.tensor(0.0).unsqueeze(0).repeat(n).to(pred_depths.device)
        ...
        for i in range(n):
            ...
            loss[i] = torch.sum(torch.abs(random_target_cos - random_input_cos)) / (random_target_cos.shape[0] + 1e-8)
